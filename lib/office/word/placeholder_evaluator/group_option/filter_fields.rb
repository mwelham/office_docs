class PlaceholderEvaluator::GroupOption::FilterFields
  #{{ loltest | list, show_lables: false, filter_fields: [a, b: [x y z], c, d, e, f, g] }}

  def apply_option
    if placeholder.field_value.present?
      placeholder.field_value = placeholder.field_value.map{|v| apply_field_filter_to_group_fields(params, v) }
    end
  end
  ...
  def restrict_fields(allowed_fields, fields)
    ...
    allowed_fields.each do |field|
      ...
      if field.is_a?(String) || !PlaceholderEvaluator::Placeholder.is_group_answer?(fields[field_name])
