class PlaceholderEvaluator::GroupPlaceholder < PlaceholderEvaluator::Placeholder
  attr_accessor :group_generation_options, :form_xml_def

  def initialize(field_identifier, field_value, options_string, form_xml_def)
  ...
  def replacement
    ...
    self.field_value = Array(field_value) if field_value.is_a? Hash

    field_options.each do |o|
      o.apply_option(self)
    end

    if group_generation_options[:generation_method] == :list
  ...
  def parse_options(options_in_string_format)
    ...
    option_objects = whole_options.map{|o| PlaceholderEvaluator::GroupOption.build_option_object(o)}
  end
