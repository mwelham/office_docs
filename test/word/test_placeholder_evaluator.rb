...
class PlaceholderEvaluatorTest < Test::Unit::TestCase
  def test_parse_field_filter
    ...
    result = evaluator.parse_field_filter("filter_fields: [a, b, c, d: [e,f: [x,y,z],g], x]")
    assert result == ["a", "b", "c", {"d"=>["e", {"f"=>["x", "y", "z"]}, "g"]}, "x"]

    result = evaluator.parse_field_filter("filter_fields:    [a,b,c,  d:   [e, g,  f:  [x,y,z]],  x]")
  ...
  def test_apply_field_filter_to_group_fields
    ...
    result = evaluator.apply_field_filter_to_group_fields("filter_fields: [a, b: [c,d], e: [p], f", field_value)
  ...
  def test_apply_field_filter_to_group_fields_for_blank_filter
    evaluator = Word::PlaceholderEvaluator.new({})
    field_value = {'a' => '1', 'b' => [{'c' => 3, 'd' => 4, 'x' => 5}, {'c' => 13, 'd' => 14, 'x' => 15}], 'e' => 6, 'y' => 8, 'z' => 9}
    result = evaluator.apply_field_filter_to_group_fields("filter_fields:", field_value)
    assert result == {}
  ...
  def test_split_options_on_commas
    ...
    options = "list, weo: test, filter_fields: [a, b: [c,d], e: [p], f], test, other_option: heh, wah: [lol, yo]"
    results = evaluator.split_options_on_commas(options)
  ...
  def test_change_multi_select_to_array
    evaluator = Word::PlaceholderEvaluator.new({})
    results = evaluator.apply_options_to_normal_value("one,two,three", ["each_answer_on_new_line"])
